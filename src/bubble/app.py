...
class AppDelegate(NSObject):
    ...
    def dismissToast_(self, timer):
        try:
            ...
            try:
                parent = getattr(toast, '_bb_parent', None)
                if parent is not None:
                    cnt = int(getattr(parent, '_bb_toast_count', 1))
                    setattr(parent, '_bb_toast_count', max(0, cnt - 1))
            except Exception:
                pass
        except Exception:
            pass
    ...
    def _show_small_toast(self, text: str, icon: str = None, color: object = None, duration: float = 1.8):
        """显示一个小号淡色圆角 toast，默认右上角，带图标，支持叠放。"""
        parents = []
        try:
            if bool(getattr(self, 'is_multiwindow_mode', False)) and getattr(self, 'multiwindow_manager', None):
                mw = self.multiwindow_manager
        ...
        for parent in parents:
            try:
                ...
                icon_sz = 16.0
                # 量测文字宽度（采用最终字号）
                meas = NSTextField.alloc().initWithFrame_(NSMakeRect(0, 0, 10, 10))
                meas.setBezeled_(False); meas.setDrawsBackground_(False); meas.setEditable_(False); meas.setSelectable_(False)
                try:
                    meas.setFont_(NSFont.systemFontOfSize_(13))
                except Exception:
                    pass
                meas.setStringValue_(str(text) if text is not None else "")
                try:
                    meas.sizeToFit()
                    lw = float(ceil(meas.frame().size.width + 2.0))
                except Exception:
                    lw = 160.0
                lw = max(20.0, min(320.0, lw))
                ...
                slot = int(getattr(parent, '_bb_toast_count', 0))
                if is_drag:
                    x = max(pad, bounds.size.width - w - pad) - slot * 4.0
                    y = max(0.0, (bounds.size.height - h) / 2.0 - slot * 2.0)
                else:
                    x = max(pad, bounds.size.width - w - pad)
                    y = max(pad, bounds.size.height - h - pad - slot * (h + 6.0))

                toast = NSView.alloc().initWithFrame_(NSMakeRect(x, y, w, h))
                toast.setWantsLayer_(True)
                try:
                    toast.layer().setCornerRadius_(12.0)
                    toast.layer().setBackgroundColor_(NSColor.colorWithCalibratedWhite_alpha_(1.0, 0.88).CGColor())
                ...
                try:
                    ...
                    img = None
                    try:
                        img = NSImage.imageWithSystemSymbolName_accessibilityDescription_(icon or 'checkmark.circle.fill', None)
                        if img is not None:
                            try:
                    ...
                    if img is not None:
                        iv.setImage_(img)
                        try:
                            if color is not None and hasattr(iv, 'setContentTintColor_'):
                ...
                # 文本
                lbl = NSTextField.alloc().initWithFrame_(NSMakeRect(pad + icon_sz + 6.0, (h - 16.0) / 2.0, lw, 16.0))
                lbl.setBezeled_(False); lbl.setDrawsBackground_(False); lbl.setEditable_(False); lbl.setSelectable_(False)
                try:
                ...
                try:
                    ...
                    if hasattr(lbl, 'setLineBreakMode_'):
                ...
                # 叠放计数标记，便于移除时回收
                try:
                    toast._bb_parent = parent
                    toast._bb_slot = slot
                    setattr(parent, '_bb_toast_count', slot + 1)
                except Exception:
                    pass
            except Exception:
                pass
    ...
    def applicationDidFinishLaunching_(self, notification):
        ...
        print("DEBUG: 设置应用图标...")
        # Packaged app places assets under NSBundle.mainBundle().resourcePath()
        # Dev mode keeps them under this module directory. Detect both.
        script_dir = os.path.dirname(os.path.abspath(__file__))
        bundle = None
        bundle_res = None
        try:
            bundle = NSBundle.mainBundle()
            bundle_res = str(bundle.resourcePath()) if bundle else None
        except Exception:
            bundle_res = None
        ...
        # Candidate locations (prefer bundle Resources when packaged)
        icns_candidates = [
            _r(bundle_res, "logo", "icon.icns") if bundle_res else None,
            _r(script_dir, "logo", "icon.icns"),
        ]
        png_hi_candidates = [
            _r(bundle_res, "logo", "icon.iconset", "icon_512x512@2x.png") if bundle_res else None,
            _r(script_dir, "logo", "icon.iconset", "icon_512x512@2x.png"),
        ]
        png_lo_candidates = [
            _r(bundle_res, "logo", "icon.iconset", "icon_512x512.png") if bundle_res else None,
            _r(script_dir, "logo", "icon.iconset", "icon_512x512.png"),
        ]
        self._icon_icns_path = next((p for p in icns_candidates if p), None)
        hi_png = next((p for p in png_hi_candidates if p), None)
        lo_png = next((p for p in png_lo_candidates if p), None)
        self._icon_png_path = hi_png or lo_png
        ...
        # 稍后自动打开第一个已启用平台的窗口（若存在），避免先落到主页
        try:
            NSTimer.scheduledTimerWithTimeInterval_target_selector_userInfo_repeats_(
                0.40, self, 'autoOpenFirstWindowIfAny:', None, False
            )
        except Exception:
            pass
    ...
    def autoOpenFirstWindowIfAny_(self, _):
        try:
            if getattr(self, '_auto_open_done', False):
                return
            ...
            enabled = []
            try:
                enabled = pm.get_enabled_platforms() or []
            except Exception:
                enabled = []
            if enabled:
                pid = getattr(enabled[0], 'platform_id', None)
                if pid:
                    mw.createWindowForPlatform_(pid)
    ...
    def applicationDidBecomeActive_(self, notification):
        ...
        try:
            ...
            # 3) 文件系统（开发模式兜底）
            if brand_img is None:
                # Dev mode fallbacks: new path under src/bubble/logo
                color_candidates = [
    ...
    def _get_favicon_image(self, platform_id, url):
        try:
            ...
            try:
                ...
                img = None
            # 1b) 开发模式：src/bubble/assets/icons
            if img is None:
                try:
                    script_dir = os.path.dirname(os.path.abspath(__file__))
    ...
    def _ensure_error_overlay(self):
        try:
            ...
            try:
                # 更高透明度的柔和背景蒙层（更通透）
                overlay.layer().setBackgroundColor_(NSColor.colorWithCalibratedWhite_alpha_(1.0, 0.6).CGColor())
            except Exception:
                pass
            ...
            try:
                ...
                card.layer().setBorderColor_(NSColor.colorWithCalibratedWhite_alpha_(0.82, 1.0).CGColor())
                card.layer().setBorderWidth_(1.0)
                card.layer().setShadowOpacity_(0.12)
                card.layer().setShadowRadius_(16.0)
            except Exception:
                pass
            overlay.addSubview_(card)

            # 标题与说明（标题稍微下移，避免与左上角返回按钮重叠）
            title = NSTextField.alloc().initWithFrame_(NSMakeRect(16, card_h - 60, card_w - 32, 22))
            title.setBezeled_(False)
            title.setEditable_(False)
            title.setSelectable_(False)
            title.setDrawsBackground_(False)
            try:
                t = self._i18n_or_default('error.navigationTitle', '加载失败')
                ...
                title.setStringValue_("加载失败")
            card.addSubview_(title)

            # 提示语更简洁可读
            label = NSTextField.alloc().initWithFrame_(NSMakeRect(16, 56, card_w - 32, 48))
            ...
            # 按钮：中间一个黑色“重试”，左上角一个“返回”（去掉中间的返回主页按钮）
            retry_title = self._i18n_or_default('button.retry', '重试')
            back_title = self._i18n_or_default('button.back', '返回')

            # 居中的重试按钮（黑底白字，圆角）
            retry_w = 160
            retry_h = 32
            retry_x = (card_w - retry_w) / 2
            retry_y = 16
            retry = NSButton.alloc().initWithFrame_(NSMakeRect(retry_x, retry_y, retry_w, retry_h))
            try:
                retry.setBordered_(False)
                retry.setBezelStyle_(NSBezelStyleRounded)
            except Exception:
                pass
            try:
                retry.setWantsLayer_(True)
                retry.layer().setCornerRadius_(8.0)
                retry.layer().setBackgroundColor_(NSColor.blackColor().CGColor())
            except Exception:
                pass
            try:
                from AppKit import NSAttributedString, NSForegroundColorAttributeName
                retry.setAttributedTitle_(NSAttributedString.alloc().initWithString_attributes_(retry_title, {NSForegroundColorAttributeName: NSColor.whiteColor()}))
            except Exception:
                retry.setTitle_(retry_title)
            try:
            ...
            # 左上角返回按钮（文字按钮，轻量）
            back_btn = NSButton.alloc().initWithFrame_(NSMakeRect(12, card_h - 32, 64, 22))
            try:
                back_btn.setBordered_(False)
                back_btn.setBezelStyle_(NSBezelStyleInline)
            except Exception:
                pass
            try:
                from AppKit import NSAttributedString, NSForegroundColorAttributeName
                back_btn.setAttributedTitle_(NSAttributedString.alloc().initWithString_attributes_(back_title, {NSForegroundColorAttributeName: NSColor.colorWithCalibratedWhite_alpha_(0.2, 1.0)}))
            except Exception:
                back_btn.setTitle_(back_title)
            try:
                back_btn.setTarget_(self)
                back_btn.setAction_(objc.selector(self.errorGoHome_, signature=b'v@:@'))
            except Exception:
                pass
            card.addSubview_(back_btn)
    ...
    def _show_error_overlay(self, message: str):
        try:
            ...
            if not self.error_overlay:
                return
            # 文案（简洁可读，不拼接底层错误信息）
            text = self._i18n_or_default('error.navigationFailed', '网络连接异常或页面无法加载。')
            
            extra = self._i18n_or_default('error.navigationHint', '请检查网络后点击“重试”。')
            text = f"{text}\n{extra}"
